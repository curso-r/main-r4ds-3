...
isso aqui. O erro mais frequente ocorre na leitura de CSVs exportados pelo
Excel: normalmente o _encoding_ deles são `latin1` ou `windows-1252`.
...
preciosos sobre os dados e a metodologia por trás deles.
...
Como o nome sugere, `NA`s explícitos podem ser observados diretamente na forma
de `NA`s no data frame. Eles normalmente aparecem em tabelas que já foram
preenchidas no formato tidy ou que foram convertidas para este formato. Neste
cenário, tratar um `NA` significa removê-lo da maneira menos disruptiva
possível.
...
quando tiramos os `NA`s de uma categoria com poucos elementos. Neste caso,
podemos usar a função `count()` do pacote dplyr com `.drop = FALSE` para revelar
os níveis com 0 representantes.

```{r, eval = FALSE}
"data-raw/caes.csv" |>
  read_csv() |>
  mutate(especie = fct(especie, c("Cão", "Gato"))) |>
  count(especie, .drop = FALSE)
```
...
omitirá a categoria varia, dando a impressão de que ela não existe.
